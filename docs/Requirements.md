...
| Требование | Комментарий |
|:---|:---|
|R9.1 Сервер при доступе по URL localhost:8080 должен выдавать html страницу загруженную из ресурсов| [Для приложения используйте reactjs](https://facebook.github.io/react/docs/tutorial.html) |
|R9.2 Через AJAX должен запрашиваться список пользователей и выводиться в таблицу| [ajax](http://api.jquery.com/jquery.getjson/) |
|R9.3 При клике на строке в таблице пользователей через AJAX должен загружаться список прав доступа для данного юзера||
|R9.4 При клике на строке в таблице прав доступа через AJAX должен загружаться список действий ассоциированных с данным правом доступа||
|R9.5 На странице должна быть форма со всеми полями из командной строки||
|R9.6 Форма должна отправлять JSON в виде POST запроса на сервлет /ajax/activity и в ответ получать статус||
|R9.7 После запроса форма должна обновить список активностей AJAX запросом||

## Requirements #9 WebPack

| Требование | Комментарий |
|:---|:---|
| Код переписать на typescript |вынести фронтовую часть в отдельную папочку |
| собирать при помощи вебпака |вынести фронтовую часть в отдельную папочку |
| копировать результат внутрь приложения скриптом |Скрипт обычный баш |

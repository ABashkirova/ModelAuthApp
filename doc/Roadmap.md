...
1. Написать `main`, считывающий параметры (R1.8):
    1. Написать фукнцию, проверяющую наличие аргументов
    2. Создаем метод печати справки `printHelp()` и `System.exit(1)` (R1.8.1, R1.8.2, R1.8.6)
    3. Создать функцию, проверяющую необходима ли справка `needHelp() -> Bool` (`arg[0] equal "-h"`)
    4. Печатаем во всех остальных случаях справку и код 0 (`System.exit(0)`)
    5. обработка в любом порядке (R1.10)
